...
const mediaLinkOption_linkEdit = document.querySelector('#link-option--edit');

mediaLinkBox.addEventListener('click', function (e) {
  if (
    e.target.parentNode.className == 'media-link__box' &&
    e.target.className == 'media-link__icon'
  ) {
    mediaLinkOption.style.display = 'block';

    mediaLinkOption_open.href = e.target.parentNode.dataset.link;
    mediaLinkOption_linkEdit.href = `/link/${e.target.parentNode.id}`;
